...
function mapSearchResultToProduct(searchResult: SearchResultProduct): Product {
  ...
  const ingredientInfos =
    source.specificationinfolist?.specificationinfo.ingredientset?.ingredients?.ingredientinfo ??
    [];
  const ingredients: Ingredient[] = ingredientInfos.map((ing) => {
    const order = ing.sequence ? Number.parseInt(ing.sequence, 10) : undefined;
  ...
  const allergenInfos =
    source.specificationinfolist?.specificationinfo.allergenset?.allergens.allergeninfo ?? [];
  ...
  const nutrientInfos =
    source.specificationinfolist?.specificationinfo.nutrientset?.nutrientinfolist.nutrientinfo ??
    [];
  const nutrients: Nutrient[] = [];

  for (const nutrient of nutrientInfos) {
    if (!nutrient.name) {
      continue;
    }

    const hasNumericValue = nutrient.decimalvalue || typeof nutrient.value === 'number';
    if (!hasNumericValue || !nutrient.unitofmeasurename?.value) {
      continue;
    }
    ...
    nutrients.push({
      ...
      value:
        ...
        (typeof nutrient.value === 'number' ? nutrient.value.toString() : ''),
  ...
  const specificationinfolist =
    ingredients.length || allergens.length || nutrients.length
      ? {
          specificationinfo: {
            ingredientset: ingredients.length ? { ingredient: ingredients } : undefined,
            allergenset: allergens.length ? { allergen: allergens } : undefined,
            nutrientset: nutrients.length ? { nutrient: nutrients } : undefined,
          },
        }
      : undefined;
  ...
  const logisticinfolist = source.logisticinfolist
    ? {
        logisticinfo: {
          assetinfolist: source.logisticinfolist.logisticinfo.assetinfolist,
        },
      }
    : undefined;
  ...
  return {
    product: {
      ...
      summary: {
        ...
        lastupdatedon: new Date(source.summary.lastupdatedon as unknown as string),
      },
